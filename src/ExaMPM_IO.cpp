...
ExaMPM::IO::IO(int argc, char** argv) {
  //! Logger
...
std::string ExaMPM::IO::file_name(const std::string& filename) {
...
bool ExaMPM::IO::check_file(const std::string& filename) {
  bool status = false;
...
/*std::string ExaMPM::IO::output_folder() const {
...
// Not sure if entity sets setup should still be utliized 
/*//! Return map of entity sets from the JSON file
tsl::robin_map<ExaMPM::Index, std::vector<ExaMPM::Index>> ExaMPM::IO::entity_sets(
  ...
  tsl::robin_map<ExaMPM::Index, std::vector<ExaMPM::Index>> entity_sets;
  ...
  try {
    ...
    if (sets.size() > 0) {
      for (Json::iterator itr = sets.begin(); itr != sets.end(); ++itr) {
        ...
        // Get the entity ids
        std::vector<ExaMPM::Index> entity_ids = (*itr).at("set");
        // Add the entity set to the list
        entity_sets.insert(
            std::pair<ExaMPM::Index, std::vector<ExaMPM::Index>>(id, entity_ids));
      }
    }
    sets_file.close();
...
//! Return analysis
std::string ExaMPM::IO::analysis_type() const {
  return json_["analysis"]["type"].template get<std::string>();
}

//! Return json analysis object
Json ExaMPM::IO::analysis() const { return json_["analysis"]; }

//! Return json object
Json ExaMPM::IO::json_object(const std::string& key) const {
  Json empty;
...
//Json ExaMPM::IO::post_processing() const { return json_["post_processing"]; }

//! Return JSON analysis boolean
bool ExaMPM::IO::analysis_bool(const std::string& key) const {
